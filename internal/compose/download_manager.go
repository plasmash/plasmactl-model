...
func (m DownloadManager) Download(ctx context.Context, c *Composition, targetDir string) ([]*Package, map[string][]string, error) {
...
func (m DownloadManager) recursiveDownload(ctx context.Context, yc *Composition, packages []*Package, parent *Package, targetDir string, seen map[string]resolvedPkg, stack *depStack) ([]*Package, error) {
	for _, d := range yc.Dependencies {
		...
		default:
			...
			if existing, ok := seen[name]; ok {
				if existing.pkgType != pkg.GetType() || existing.url != pkg.GetURL() || existing.ref != ref {
					return packages, fmt.Errorf(
						...
						pkg.GetType(), pkg.GetURL(), ref, pkgRequiredBy(parent),
					)
				}
				// Same source already resolved — accumulate the additional parent and skip.
				existing.requiredBy = append(existing.requiredBy, pkgRequiredBy(parent))
				seen[name] = existing
				continue
			}

			seen[name] = resolvedPkg{pkgType: pkg.GetType(), url: pkg.GetURL(), ref: ref, requiredBy: []string{pkgRequiredBy(parent)}}
			...
			// If package has compose.yaml, recurse into it.
...
// pkgRequiredBy returns the name of the parent package, or "root" if declared in the root compose.
func pkgRequiredBy(parent *Package) string {
	if parent == nil {
		return "root"
	}
