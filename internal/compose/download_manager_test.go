...
func newTestDM() DownloadManager {
	return CreateDownloadManager(&keyringWrapper{
		keyringService: keyring.NewService(keyring.NewFileStore(nil), nil),
	})
}
...
// writeTestCompose writes a compose.yaml into dir.
func writeTestCompose(t *testing.T, dir string, deps []Dependency) {
	t.Helper()
	c := Composition{Dependencies: deps}
...
// writes a compose.yaml with the given dependencies.
...
// pathDep builds a Dependency with type "path".
...
func TestDownloadManager(t *testing.T) {
	tests := []struct {
		name      string
		setup     func(t *testing.T) *Composition
	...
	}{
		{
			...
			setup: func(_ *testing.T) *Composition {
				return &Composition{Dependencies: []Dependency{
