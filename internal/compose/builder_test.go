...
func makeEntry(prefix, path, from string) *fsEntry {
	return &fsEntry{Prefix: prefix, SrcPath: path, DstPath: path, Entry: fakeFileInfo{name: path}, From: from}
}
...
// buildItems simulates what build() does: topsort from platform.
...
func TestDependencyGraph(t *testing.T) {
	tests := []struct {
		...
		{
			// Simulates a realistic compose.yaml:
			//
			//   dependencies:
