...
func TestLookupLock(t *testing.T) {
    ...
    - name: pkg-a
      ...
      ref: v1.0.0
      path: .plasma/model/compose/packages/pkg-a/v1.0.0
      required_by:
        - root
    - name: lib-common
      type: git
      ...
      ref: v0.5.0
      path: .plasma/model/compose/packages/lib-common/v0.5.0
      required_by:
        - pkg-a
        - pkg-b
`
	...
	t.Run("round-trip: buildLock → writeLock → LookupLock", func(t *testing.T) {
		...
		packagesDir := filepath.Join(platformDir, ".plasma", "model", "compose", "packages")
		targetDir := t.TempDir()
